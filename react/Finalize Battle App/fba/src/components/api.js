...
export const getAllUserData = (playerOneName, playerTwoName) => {
  ...
  return axios
    .all([dataPlayerOne, dataPlayerSecond])
    .then(([firstPlayerData, secondPlayerData]) => {
      ...
      const starsFirstPlayer = reposFirstPlayer.reduce(
        (total, item) => total + item.stargazers_count,
        0
      );
      const starsSecondPlayer = reposSecondPlayer.reduce(
        (total, item) => total + item.stargazers_count,
        0
      );

      const scoreFirstPlayer = starsFirstPlayer + profileFirstPlayer.followers;
      const scoreSecondPlayer =
        starsSecondPlayer + profileSecondPlayer.followers;
